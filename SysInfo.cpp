...
void CSysInfo::DetectIEVersion()
{
	...
	if (dwSize>0) {
		...
		swscanf_s(szKeyValue, _T("%d.%d.%d.%d"), &m_dwIEMajor, &m_dwIEMinor, &m_dwIEBuild, &m_dwIESomething);
	};
}
...
void CSysInfo::DetectADOVersion()
{
	...
	if (dwSize>0) {
		...
		swscanf_s(szKeyValue, _T("%d.%d.%d.%d"), &m_dwADOMajor, &m_dwADOMinor, &m_dwADOBuild, &m_dwADOSomething);
	};
...
void CSysInfo::DetectUserAdmin()
{
    ...
    __try
    {
        ...
        do
        {
            ...
            if( !pInfoBuffer )
                __leave;
			memset(pInfoBuffer, 0, dwInfoBufferSize);
            SetLastError( 0 );
	 	...
	 	DWORD *v_build, DWORD *v_something, DWORD *size)
{
	*v_major=0; *v_minor=0; *v_build=0; *v_something=0; *size=0;
	...
	if (i>0)
	{
		...
		wcscpy_s(lpFileName, i, lpszFileName);
	}
	else
		return FALSE;
...
void CSysInfo::GetMIBAdapterIPStr(int i, LPTSTR lpIPStr)
{
	...
	swprintf_s(szIPTmp,20, _T("%d.%d.%d.%d"), pInfo->IP[0], pInfo->IP[1], pInfo->IP[2], pInfo->IP[3]);
	//_tcscpy(lpIPStr,szIPTmp);
	wcscpy_s(lpIPStr,20, szIPTmp);
}
...
void CSysInfo::GetMIBAdapterMaskStr(int i, LPTSTR lpMaskStr)
{
	...
	swprintf_s(szMaskTmp,20, _T("%d.%d.%d.%d"), pInfo->SubnetMask[0], pInfo->SubnetMask[1], pInfo->SubnetMask[2], pInfo->SubnetMask[3]);
	//_tcscpy(lpMaskStr,szMaskTmp);
	wcscpy_s(lpMaskStr,20, szMaskTmp);
}
